...
import { productviewallAPI } from "../services/productServices";
...
const Accessories = () => {
  ...
  if (isLoadingProducts) return <LoadingWrapper>Loading accessories...</LoadingWrapper>;
  if (isErrorProducts) return <ErrorWrapper>Error: {productsError.message}</ErrorWrapper>;

  return (
    <AccessoriesWrapper>
      <PageHeader>
        <h1>Accessories Collection</h1>
        <Subtitle>Discover our curated collection of trendy accessories</Subtitle>
      </PageHeader>

      <CategoryNav>
        {["All", "Watches", "Bags", "Sunglasses", "Jewelry", "Others"].map(category => (
          <CategoryButton 
            key={category}
